...
def latlon_to_pixel(lat, lon, shape):
    height, width = shape
...
def pixel_distance_to_meters(dx, dy, lat):
...
def compute_line_of_sight_map(
    ...
    pos_x, pos_y = latlon_to_pixel(*position_latlon, topology_map.shape)
    tgt_x, tgt_y = latlon_to_pixel(*target_latlon, topology_map.shape)

    coordinates = {}
    max_angle = -np.inf

    points = _bresenham_line(pos_x, pos_y, tgt_x, tgt_y)
    ...
    for x, y in points:
        ...
        distance = pixel_distance_to_meters(dx, dy, position_latlon[0])
